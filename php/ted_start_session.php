...
$data_array = array();
if(isset($_POST['username']) && isset($_POST['first_name']) && isset($_POST['last_name']) && isset($_POST['user_type']) && isset($_POST['marking_centre_name']) && isset($_POST['session_id']) && isset($_POST['session_name']) && isset($_POST['session_level'])){
    
    if($_POST['user_type'] == 'DEO'){
        if(attendance_status($db_ted,$_POST['username']) != 'true'){
            $data_array['status'] = '400';
            $data_array['response_msg'] = 'Unable to log user in. '.attendance_status($db_12_gce,$_POST['username']);
        }else{
        if(login_status($db_ted,$_POST['username']) == 0){
            update_login_status($db_ted,$_POST['username']);
            ...
            $_SESSION['session_year'] = $_POST['session_id'];
        }else{
        $data_array['status'] = '400';
