...
if(isset($_POST['subject']) && isset($_POST['paper']) && isset($_POST['app_belt_no'])){
        ...
        (eb.no_of_scripts / eb.no_of_examiners) * (CASE WHEN e.role = "EXAMINER" THEN mr.examiner * :rate_value WHEN e.role = "CHECKER" THEN mr.checker * :rate_value ELSE mr.t_leader * :rate_value END) AS gross_pay,
        (eb.no_of_scripts / eb.no_of_examiners) * (CASE WHEN e.role = "EXAMINER" THEN mr.examiner * :rate_value WHEN e.role = "CHECKER" THEN mr.checker * :rate_value ELSE mr.t_leader * :rate_value END) * :tax AS 15_wht,
        (eb.no_of_scripts / eb.no_of_examiners) * (CASE WHEN e.role = "EXAMINER" THEN mr.examiner * :rate_value WHEN e.role = "CHECKER" THEN mr.checker * :rate_value ELSE mr.t_leader * :rate_value END) - (eb.no_of_scripts / eb.no_of_examiners) * (CASE WHEN e.role = "EXAMINER" THEN mr.examiner * :rate_value WHEN e.role = "CHECKER" THEN mr.checker * :rate_value ELSE mr.t_leader * :rate_value END) * :tax AS net_pay
    FROM 
        examiner e INNER JOIN marking_rates mr ON (e.subject_code = mr.subject_code)
    ...
    WHERE 
        ...
        AND e.attendance = "1"
        AND e.role IN ("EXAMINER","TEAM LEADER","CHECKER")
        AND g.subject = :subject_code
        AND g.paper = :paper_no
        AND g.belt_no = :belt_no
        AND g.marking_centre = :marking_centre_code
    GROUP BY 
        e.nrc, e.tpin, full_name, e.role, mr.t_leader, mr.examiner, mr.checker, e.account_no, e.bank,eb.no_of_scripts,eb.no_of_examiners,  e.branch,g.no_of_scripts
    ');
			...
			<div class="items p-3 ">
				<div class="row justify-content-center">
					<div class="col-md-9 alert alert-info">
						<h3 class="text-center"><?php echo $subject_code,' - ',subject_name($db_12_gce,$subject_code); ?> PAPER <?php echo $paper_no; ?> BELT <?php echo $belt_no; ?></h3> 
					</div>
