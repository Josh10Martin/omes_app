...
$sql = $db_12_gce->prepare('SELECT m.exam_no AS exam_no,m.centre_code AS centre_code, su.subject_name AS subject_name,m.paper_no AS paper_no,m.question AS question,m.mark AS mark, m.date_entered AS date_entered
                        FROM marks_questions m INNER JOIN subjects su ON (m.subject_code = su.subject_code)
...
if($sql->rowCount() > 0){
    ...
    while($row = $sql->fetch(PDO::FETCH_ASSOC)){
        ...
        $data_array[$i]['question'] = $row['question'] ?? '';
