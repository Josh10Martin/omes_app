...
$sql = $db_12_gce->prepare('SELECT id, full_name,position,no_of_scripts,bank,branch,account_no,gross_pay,15_wht,net_pay,subject_name,paper_no,belt_no
...
if($sql->rowCount() > 0){
        ...
        while($row = $sql->fetch(PDO::FETCH_ASSOC)){
                ...
                $data_array[$i]['wht'] = "K".number_format((float)$row['15_wht'],2,'.','') ?? '';
                $data_array[$i]['net_pay'] = "K".number_format((float)$row['net_pay'],2,'.','') ?? '';
