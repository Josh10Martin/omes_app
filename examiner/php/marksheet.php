...
if (isset($_POST['centre']) && isset($_POST['subject']) && isset($_POST['paper']) && isset($_POST['status']) && isset($_POST['candidate_type'])) {
        ...
        if ($status == 'L' && ($belt_no == 0 && $belt_no == '' && preg_match('/^0[0-9]+$/', $belt_no))) {
                ...
                $data_array['response_msg'] = 'Enter a belt number. Not 0  or a belt starting with 0 ';
        } elseif ($status == 'L' && centre_already_entered($db_12_gce, $subject_code, $paper_no, $school, $belt_no, $candidate_type, $_SESSION['marking_centre_code']) != 'true') {
                $data_array['status'] = '400';
                $data_array['response_msg'] = centre_already_entered($db_12_gce, $subject_code, $paper_no, $school, $belt_no, $candidate_type, $_SESSION['marking_centre_code']);
        } else {
                ...
                $sql = $db_12_gce->prepare('SELECT s.subject_code AS subject_code, s.subject_name AS subject_name, sc.centre_code AS centre_code, sc.centre_name,
                                pa.paper_no AS paper_no,pa.max_mark AS max_mark, m.exam_no AS exam_no, m.first_name AS first_name,m.last_name AS last_name, 
                                        CASE WHEN m.mark = 0 AND m.status ="L" THEN "" ELSE m.mark END AS mark, m.status AS status,m.sen AS sen,m.belt_no AS belt_no,
                                m.entered_by AS entered_by,m.date_entered AS date_entered,m.disable AS disable
                                ...
                                GROUP BY s.subject_code,s.subject_name,sc.centre_code,sc.centre_name,pa.paper_no,pa.max_mark,m.exam_no,m.first_name,m.last_name,m.mark,m.status,m.entered_by,m.date_entered,m.belt_no,m.disable
