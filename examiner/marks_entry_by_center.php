			...
			<div class="items p-3 ">
                ...
                // echo $_SESSION['marking_centre_code']
					...
					<div class="col-md-4 col-sm-6">
						...
						<select name="centre" class="select"  required>
						</select>
					</div>
					<div class="col-md-4 col-sm-6">
					...
					<div class="col-md-4 col-sm-6">
						...
						<select name="paper" class="select"  required>
						</select>
					</div>
					<div class="col-md-4 col-sm-6">
            ...
            <div class="content pt-1 d-none" id="result">
                ...
                <div class="row">
                    ...
                    <form action="" method="" id="marksheet-form">    
                        ...
                        <div class="row fixed-bottom p-3 d-flex justify-content-between">
                            <div id="subject-help" class=" d-none "> <strong class="green-ecz">SUBJECT: <span class="subject_name"></span>.  PAPER: <span class="paper_no"></span> | BELT: <span class="belt_no"></span></strong>  </div>
      ...
      //  $(window).on('scroll',function(){
       ...
       var formValid = true;
        ...
        $(window).on('scroll', function() {
            ...
            if (scrollTop >= divOffset) {
              $('#subject-help').removeClass('d-none');
              $('#row-count').removeClass('d-none');
            } else {
              $('#subject-help').addClass('d-none');
              $('#row-count').addClass('d-none');
            }
          });
        ...
        buttons: [
            ...
            },
            {
                ...
                click: function(){
                    // $('#result').addClass('d-none');
					// $('#parameters').removeClass('d-none');
                    // location.href="marks_entry_by_center.php";
                    location.reload();
                    $(this).dialog('close');
                }
            }
...
// };
    ...
    $('body').on('keydown', 'input', function(e) {
        ...
        if (e.which === 13 || e.which === 40 ) {
          ...
          if(mark == ''){
            console.log("QUOT MARK")
            //mark.val('0')
            $(this).closest('tr').find('td:nth-child(5)').children().val('0');
          }
    ...
    function check_if_belted(){
        $('#parameters-form').submit(function(e){
		e.preventDefault();
        var centre_code = $('select[name=centre]').val(),
            subject_code = $('select[name=subject]').val(),
            paper_no = $('select[name=paper]').val();

        $.ajax({
            url: 'php/check_if_belted.php',
            method: 'POST',
            ...
            success:function(data){
                if(data.status == '200'){
                    $('.dialog5').text(data.response_msg+'. Do tou want to continue with marks entry?').dialog('open');
	...
	function get_marksheet(){
			...
			success: function(data){
				...
				if(data.status == '400'){
                    ...
                    $('input[type=hidden][name=chief_examiner_username]').val(data.chief_examiner_username);
                        console.log("CHIEF UNAME:",data.chief_examiner_username)
					...
					$.each(data.candidate,function(){
                        var checked = this["status"] == 'X' ? 'checked' : '',
                         mark_disable = (this["status"] == 'X' || this["disable"] == 1) ? 'disabled' : '',
						...
						'<td><input type="checkbox" name="absent" value="'+this["exam_no"]+'" id="" '+checked+' ></td>'+


						
						'<td>'+
                            '<input type="text" class="form-control mark" name="raw_mark['+this["exam_no"]+']" placeholder="0" value="'+this["mark"]+'" maxlength="3" pattern="[0-9]+" max="'+this["max_mark"]+'" style="width:40px;" />'+
        ...
        function authorise(){
            ...
            var username = $('input[type=hidden][name=chief_examiner_username]').val(),
            ...
            $.ajax({
                ...
                // url: 'http://192.9.200.86:8000/api/auth-user/api-ems/',
                method: 'POST',
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.status == '400'){
                        ...
                        $('.dialog4').text(data.response_msg+' Generate transcription checklist?').dialog('open');
                        $('buton[type=submit]').attr('disabled',false).text('Submit');
                    }
                }
