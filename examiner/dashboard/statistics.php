...
if($_SESSION['user_type'] == 'ECZ'){
    $sql = $db_12_gce->prepare('SELECT SUM(status ="-") AS no_of_entered_marks, SUM(status ="L") AS no_of_missing_marks, SUM(improvised_mark = "1") AS no_of_improvised_marks,COUNT(exam_no) AS count_rows
                            FROM marks');
    $sql->execute();
