...
$pdf = new Dompdf($options);
    ob_start();
    if((isset($_POST['centre_code']) || isset($_SESSION['centre_code'])) && (isset($_POST['subject']) || isset($_SESSION['subject_code'])) && (isset($_POST['candidate_type']) || isset($_SESSION['sen']))){

        $centre_code = $_POST['centre_code'] ??  $_SESSION['centre_code'];
        $subject_code = explode(':',$_POST['subject'])[0] ?? $_SESSION['subject_code'];
        $candidate_type = $_POST['candidate_type'] ?? $_SESSION['sen'];
        $i =0;
        $sql = $db_ted->prepare('SELECT m.exam_no AS exam_no,m.first_name AS first_name,m.last_name AS last_name,m.mark AS mark,
                                ...
                                su.subject_code AS subject_code,su.subject_name AS subject_name
                                ...
                                GROUP BY m.exam_no,m.first_name,m.last_name,m.mark,m.status,m.entered_by,sc.centre_code,sc.centre_name,su.subject_code,su.subject_name
                                ');
        $sql->execute(array(
...
<html>
        ...
        <tr >
            ...
            <td>
               <p  style="text-align: center;">EXAMINATIONS COUNCIL OF ZAMBIA</p> 
                ...
                <p style="text-align: center;">TRANSCRIPTION CHECKLIST [<?php if($candidate_type == 0){echo 'MAINSTREAM';}else {echo 'SEN';} ?>]</p> 
            </td>
        </tr>
    </table>
    ...
    <tr>
        <th>
            Marking Centre: <?php echo $_SESSION['marking_centre_code'],' - ',$_SESSION['marking_centre']; ?>    
        </th>
    </tr>
    <tr>
    ...
    <tr>
        <th>
            Session: <?php echo $_SESSION['session_name']; ?>
        </th>
        <th>
