...
<div class="page-wrapper">
    <div class="content">
        <div class="row justify-content-between px-2">
            ...
            <div class="col-xs-6">
                <a href="javascript:void(0)" data-toggle="modal" data-target="#add-admin" class="btn bg-light btn-sm"><i class="fa fa-user-plus"></i> Add ADMIN</a>
            </div>
...
$(document).ready(function(){
   ...
   $('#add_admin').submit(function(e){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $('table#table tbody').prepend('<tr class="'+username+'">'+
                    '<td id="'+username+'">'+username+'</td>'+
                    '<td>'+first_name+'</td>'+
                    '<td>'+last_name+'</td>'+
                    '<td>'+marking_centre_name+'</td>'+
                    ...
                    '<td class="text-right">'+
                        '<div class="dropdown dropdown-action">'+
                            ...
                            '<div class="dropdown-menu dropdown-menu-right">'+
                                ...
                                '<a id="'+username+'" class="dropdown-item '+activation_status+' dea_activate" href="javascript:void(0)" data-id="'+username+'" data-toggle="modal" data-target="delete_admin"><i class="fa fa-ban fa-fw red-ecz"></i> '+activation+'</a>'+
                            '</div>'+
                        '</div>'+
                    '</td>'+
                    '</tr>');
   ...
   function send_admin_email(username,email,first_name,last_name,marking_centre_name,password,user_type){
    $.ajax({
        ...
        url: 'https://verify.exams-council.org.zm/orvs/send_ted_admin_email.php',
        method: 'POST',
   ...
   function change_status(id){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if($('select[name=marking_centre]').text() == 'undefined - undefined'){
