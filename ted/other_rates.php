        ...
        <div class="content">
            ...
            <table class="table custom-table table-bordered">
                <thead >
                    ...
                    <th>CENTRE CHAIRPERSON RATE (K)</th>
                    <th>DEPUTY CENTRE CHAIRPERSON RATE (K)</th>
                    <th>ACTION </th> 
                </thead>
                <tbody>
                   <tr>
      ...
      <div class="modal fade" id="update-marking-rate" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            ...
            <div class="row">
                ...
                <div class="col-md-6 p-1">
                        <div class="d-flex justify-content-between">
                            <label>CENTRE CHAIRPERSON:</label>
                            <input type="number" name="cch" step=".01" class="mark form-control" required>
                        </div>
                    </div>
                    <div class="col-md-6 p-1">
                        <div class="d-flex justify-content-between">
                            <label>DEPUTY CENTRE CHAIRPERSON:</label>
                            <input type="number" name="dcch" step=".01" class="mark form-control" required>
                        </div>
                    </div>
                </div>
                    <!-- <div class="row">
...
$(document).ready(function(){
    ...
    if($('input[type=number][name=checker]').val() == '' && $('input[type=number][name=sad]').val() == ''){
        ...
        $('input[type=number][name=cch]').attr('required',true);
        $('input[type=number][name=dch]').attr('required',true);
    }else{
        ...
        $('input[type=number][name=cch]').attr('required',false);
        $('input[type=number][name=dcch]').attr('required',false);
    }
}
    ...
    function rates(){
        $('span.update').click(function(){
            var checker_rate = $(this).closest('tr').find('td:nth-child(2)').text(),
             ...
             cch_rate = $(this).closest('tr').find('td:nth-child(4)').text(),
                dcch_rate = $(this).closest('tr').find('td:nth-child(5)').text();
                
                // alert(id);
                // alert(che_rate);
            $('input[type=number][name=checker]').val(checker_rate);
            $('input[type=number][name=sad]').val(sad_rate);
            $('input[type=number][name=cch]').val(cch_rate);
            $('input[type=number][name=dcch]').val(dcch_rate);
            check_required();
