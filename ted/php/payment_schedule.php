...
include '../../config.php';
$data_array = array();
$sql = $db_9->prepare('SELECT ce.centre_code AS marking_centre_code,ce.name AS marking_centre_name, ex.nrc AS nrc, CONCAT(ex.title," ",ex.first_name," ",ex.last_name) AS full_name,SUM(a.script_no) AS script_no,
                        CONCAT("\'",br.sortcode) AS sortcode, CONCAT("\'",ex.account_no) AS account_no,

                        (SUM(a.script_no) * (CASE WHEN ex.role = "3" THEN mr.t_leader ELSE mr.examiner END) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3"))) AS gross_pay,

                        (SUM(a.script_no) * (CASE WHEN ex.role = "3" THEN mr.t_leader ELSE mr.examiner END) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3")) * (15/100)) AS 15_wht, 
                        (SUM(a.script_no) * (CASE WHEN ex.role = "3" THEN mr.t_leader ELSE mr.examiner END) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3")) - (SUM(a.script_no) * (CASE WHEN ex.role = "3" THEN mr.t_leader ELSE mr.examiner END) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3"))) * (15/100)) AS net_pay,
                        ...
                        (SUM(a.script_no) * (mr.checker)) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3")) AS gross_pay,

                       (SUM(a.script_no) * (mr.checker) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3")) * (15/100)) AS 15_wht, 

                        (SUM(a.script_no) * (mr.checker) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3"))) - (SUM(a.script_no) * (mr.checker) / (SELECT COUNT(*) FROM examiner WHERE subject_code = ex.subject_code AND paper_no = ex.paper_no AND belt_no = ex.belt_no AND marking_centre = ex.marking_centre AND province = ex.province AND role IN ("1","3"))) * (15/100) AS net_pay,
                        ...
                        SELECT ce.centre_code AS marking_centre_code, ce.name AS marking_centre_name, u.nrc AS nrc, CONCAT(u.first_name," ",u.last_name) AS full_name,SUM(m.status = "-") AS script_no,  CONCAT("\'",br.sortcode) AS sortcode, CONCAT("\'",u.account_no) AS account_no, 
                        SUM(m.status = "-") * mr.data_entry AS gross_pay, (SUM(m.status = "-") * mr.data_entry) * (15/100) AS 15_wht, (SUM(m.status = "-") * mr.data_entry) - (SUM(m.status = "-") * mr.data_entry) * (15/100) AS net_pay, 
...
if($sql->rowCount() > 0){
        ...
        // $data_array[0]['nrc'] = $row['nrc'] ?? '';
        // $data_array[0]['payee_full_name'] = $row['full_name'] ?? '';
        // $data_array[0]['sortcode'] = $row['sortcode'] ?? '';
        // $data_array[0]['account_no'] = $row['account_no'] ?? '';
        // $data_array[0]['gross_pay'] ="'".number_format((float)$row['gross_pay'],2,'.','') ?? '';
        // $data_array[0]['15_wht'] = "'".number_format((float)$row['15_wht'],2,'.','') ?? '';
        // $data_array[0]['net_pay'] = "'".number_format((float)$row['net_pay'],2,'.','') ?? '';
        // $data_array[0]['bank'] = $row['bank'] ?? '';
        // $data_array[0]['branch'] = $row['branch'] ?? '';
        // $data_array[0]['subject_code'] = $row['subject_code'] ?? '';
        // $data_array[0]['paper_no'] = $row['paper_no'] ?? '';
        // $data_array[0]['belt_no'] = $row['belt_number'] ?? '';
        $i =0;
