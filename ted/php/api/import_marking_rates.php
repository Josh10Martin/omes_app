...
foreach($data as $key => $row){
    if(isset($row['subject_code'])){
        ...
        try{
        $sql =$db_ted->prepare('INSERT INTO marking_rates(subject_code,paper_no,t_leader,examiner,checker,data_entry)
                                    VALUES(:subject_code,:paper_no,:t_leader,:examiner,:checker,:data_entry)
                                    ON DUPLICATE KEY UPDATE
                                   
                                    t_leader = VALUES(deputy_c_examiner),
