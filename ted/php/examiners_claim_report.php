...
$pdf = new Dompdf($options);
    ...
    if(isset($_POST['subject']) && isset($_POST['paper']) && isset($_POST['app_belt_no'])){
        ...
        $grand_total = 0;
        $no_of_examiners =0;
        $sql = $db_9->prepare('SELECT ex.title AS title, CONCAT(ex.first_name," ",ex.last_name) AS full_name,mr.examiner AS paper_rate,SUM(a.script_no)
         AS script_no, mr.examiner * SUM(a.script_no) AS amount_claimed,ex.account_no AS account_no,
                                b.name AS bank,br.name AS branch,COUNT(DISTINCT(ex.nrc)) AS no_of_examiners
                                ...
                                GROUP BY ex.title,ex.first_name,mr.examiner,ex.last_name,ex.nrc,ex.account_no,b.name,br.name');
        $sql->execute(array(
