...
$sql = $db_ted->prepare('SELECT id, examiner_number,first_name,last_name, CASE WHEN activation_status = 1 THEN "ACTIVE" ELSE "NOT ACTIVE" END AS active,
                            CASE WHEN login_status = 0 THEN "LOGGED OUT" ELSE "LOGGED IN" END AS login_status
                            FROM examiner WHERE marking_centre =:marking_centre_code AND role = "DATA ENTRY OFFICER" AND attendance = 1 AND session =:session');
$sql->execute(array(
