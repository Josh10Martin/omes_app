...
if(isset($_POST['email'])){
        ...
        if(username_exists($db_ted,$username) != 'false'){
                $data_array['status'] = '400';
                $data_array['response_msg'] = 'username '.username_exists($db_ted,$username).' is in use. Please choose another username';
        }else{
                try{
                        $sql = $db_ted->prepare('INSERT IGNORE INTO users (username,email,password,first_name,last_name,user_type,marking_centre,added_by)
                                                VALUES(:username,:email,:password,:first_name,:last_name,:user_type,:marking_centre,:added_by)');
                        $sql->execute(array(
                                ':username'=>$username,
                                ':email'=>$email,
