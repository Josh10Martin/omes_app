...
<!-- <div id="delete_center" class="modal fade delete-modal" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
      ...
      <div class="modal fade" id="add-marking-center" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            ...
            <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>CODE:</label>
                            <input type="text" name="centre_code" class="form-control" value="<?php echo marking_centre_code($db_9); ?>" required readonly>
                        </div>
                    </div>
        ...
        <div class="modal fade" id="edit-marking-center" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
            <form action="" method="post" id="edit_marking_centre">
            <div class="modal-header p-2 bg-success ">
                <div class="modal-title text-white h5 text-center">Edit Marking Centre</div>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            ...
            <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>CODE:</label>
                            <input type="text" name="centre_code" class="form-control" value="" required readonly>
                        </div>
                    </div>
...
function add_marking_centre(){
    ...
    $('#add_marking_centre').submit(function(e){
        ...
        var centre_code = $('input[type=text][name=centre_code]').val(),
            ...
            centre_type = $('select[name=centre_type]').val()  == 'E' ? 'GRADE 9 EXTERNAL' : 'GRADE 9 INTERNAL';
        $.ajax({
...
function get_all_marking_centres(){
    $.ajax({
            ...
            success:function(data){
                   ...
                   '</tr>' );
                    ...
                    '<td class="text-right">'+
                            '<div class="dropdown dropdown-action">'+
                                '<a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>'+
                                '<div class="dropdown-menu dropdown-menu-right">'+
                                    '<a class="dropdown-item edit" href="javascript:void(0)" data-toggle="modal" data-target="#edit-marking-center" ><i class="fa fa-pencil m-r-5 blue-ecz"></i> Edit</a>'+
               ...
               edit_marking_centres();
                }
            }
        });
}

function edit_marking_centres(){
    $('a.edit').click(function(){
        var marking_centre_code = $(this).closest('tr').find('td:nth-child(1)').html(),
         marking_centre_name = $(this).closest('tr').find('td:nth-child(2)').html();
         $('form#edit_marking_centre input[type=text][name=centre_code]').val(marking_centre_code);
         $('form#edit_marking_centre input[type=text][name=centre_name]').val(marking_centre_name);
       
    });
    $('form#edit_marking_centre').submit(function(e){
        e.preventDefault();

        $.ajax({
            url: 'php/edit_marking_centre.php',
            method: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
            beforeSend: function(){
