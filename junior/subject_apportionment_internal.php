...
function subject_apportionment(){
    $('#apportion_subjects').submit(function(e){
        ...
        if($('input[type=checkbox][name=centre]:checked').length > 0 && $('input[type=checkbox][name=subject_schools]:checked').length > 0){
         
            
                $('input[type=checkbox][name=centre]:checked').each(function(){
                centre_code.push($(this).val());
            });
            $('input[type=checkbox][name=subject_schools]:checked').each(function(){
            ...
            $('.dialog').text('You need to choose both the examination centre(s) and subject(s)').dialog('open');
        }
    });
}
});
