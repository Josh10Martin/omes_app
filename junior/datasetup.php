      ...
      <!-- Preloader -->
            ...
            <div class="content">
                ...
                <div class="row">
                        ...
                        <div class="col-md-6">
                          <a href="marking-sessions.php"><div><i class="fa fa-eye" aria-hidden="true"></i> VIEW / UPDATE SESSION:</div></a>   
                     </div>
                        <!-- <div class="col-md-6">
                ...
                <div class="row">
                        <div class="col-md-6">
                             <a href="marking_rates.php"><div><i class="fa fa-eye" aria-hidden="true"></i> VIEW / UPDATE MARKING RATES:</div></a>   
                        </div>
                        <!-- <div class="col-md-6">
                ...
                <div class="row">
                        <div class="col-md-6">
                             <a href="all-ocrs-centers.php"><div><i class="fa fa-eye" aria-hidden="true"></i> VIEW EXAMINATION CENTRES:</div></a>   
                        </div>
                        <div class="col-md-6">
                               <a href="javascript:void(0)" data-toggle="modal" data-target="#import-ocrs-centers-modal"><div><i class="fa fa-cloud-upload" aria-hidden="true"></i> IMPORT EXAMINATION CENTRES</div></a> 
                        </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            ...
            <div class="modal-body">
              ...
              <div class="row  d-none" id="by-marking-centre-row">
                ...
                <div class="col-md-12">
                  <label for="centre"> Select Marking Centre</label>
                  <select name="marking_centre" class="select" id="centre" required>
                    
                  </select>
                </div>
                <div class="col-md-6">
                  <label for="centre"> Select Subject</label>
                  <select name="subject" class="select" id="subject" required>
                    
                  </select>
                </div>
                <div class="col-md-6">
                  <label for="centre"> Select Paper number</label>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            ...
            <div class="modal-body">
              ...
              <div class="col-md-12">
                ...
                <div class="col-md-12">
                  <label for="centre"> Select Marking Centre</label>
                  <select name="marking_centre" class="select" id="centre">
               
                  </select>
                </div>
                <div class="col-md-12">
                  <label for="centre"> Select Examination Centre</label>
                  <select name="centre_code" class="select" id="centre">
                  </select>
                </div>
              ...
              <div class="row " id="by-subject-row">
                ...
                <div class="col-md-6">
                  ...
                  <select name="paper" class="select" id="">
                  </select>
                </div>
              </div>
        ...
        <form action="" type="post" style="width:100%;" id="upload_form" enctype="multipart/form-data">
          <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="file" accept=".csv"class="form-control" placeholder="choose file to upload" id="myFile" accept=".csv" name="myFile" required>
            </div>
            <div class="feedback1" style="text-align:center;"></div>
            <div class="feedback2" style="text-align:center;"></div>
            <div class="modal-footer">
              <input type="hidden" name="centre_type" value ="<?php echo $_SESSION['session_type'] ?>">
              <button type="button" id="close" class="btn btn-secondary mr-auto" data-dismiss="modal">Close</button>
              <img class="loading" src="../images/loading.gif" style="transform: translateX(100%); display:none;"/>
              <button type="submit" id="save" class="btn btn-primary float-right">Upload</button>
            </div>
...
<!-- import centres -->
      <div class="modal fade" id="import-ocrs-centers-modal" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        ...
        <form action="" type="post" style="width:100%;" id="ocrs_import_form" enctype="multipart/form-data">
          <div class="modal-content">
            <div class="modal-header bg-success p-1">
              <h5 class="modal-title h4 text-white" id="exampleModalLongTitle">Import Examination Centres From OCRS</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
                <label>Category: <?php echo $_SESSION['session_type'] == 'E' ? 'Grade 9 External' : ($_SESSION['session_type'] == 'I' ? 'Grade 9 Internal' : '[NOT DEFINED]'); ?></label>
                <!-- <input type="file" accept=".csv"class="form-control" placeholder="choose file to upload" id="myFile" name="myFile" required> -->


                <!-- <select id="" class="select" name="centre_type" required>
                  <option value="" disabled>Choose Category</option>
                  <option value="I">Internal Centres</option>
                  <option value="E">External Centres</option>
                </select> -->
            </div>
            <div class="modal-footer">
              <button type="button" id="close" class="btn btn-secondary mr-auto" data-dismiss="modal">Close</button>
...
$(document).ready(function(){
  ...
  upload_examiners();
  get_province();
  get_marking_centre_for_extract();
...
function hideMarkingCenterDiv(){
  ...
  function upload_marksheet(){
    $('#upload_form').submit(function(e){
      e.preventDefault();
      var centre_type = $('input[type=hidden][name=centre_type]').val();
      $.ajax({
        ...
        beforeSend: function(data){
            ...
            $('button[id=close]').attr('disabled',true);
            $('img.loading').css('display','block');
            $('.feedback1').text('Uploading marksheet....');
        },
        success:function(data){
          if(data.status == '200'){
            // $('button[id=save]').attr('disabled',false).removeClass('bg_att');
...
function align_centres(){
  $.ajax({
    ...
    dataType: 'json',
    beforeSend:function(){
      $('.feedback1').text('Aligning any available mainstream examination centre(s) to marking centre');
    },
    success:function(data){
      if(data.status == '200'){
        // $('button[id=save]').attr('disabled',false).removeClass('bg_att');
...
function align_sen_external(){
  $.ajax({
    url: 'php/align_sen_external.php',
    method: 'POST',
    dataType: 'json',
    beforeSend:function(){
      $('.feedback1').text('Aligning sen candidate(s)....');
    },
    success:function(data){
      if(data.status == '200'){
        $('.feedback2').text(data.response_msg);
...
//     dataType: 'json',
//     beforeSend:function(){
//       ('.feedback1').text('Aligning centre(s) to respective province(s)');
//     },
//     success:function(data){
//       if(data.status == '200'){
//         // $('button[id=save]').attr('disabled',false).removeClass('bg_att');
...
// function align_provinces_internal(){
//   $.ajax({
//     url: 'php/align_provinces_internal.php',
//     method: 'POST',
//     dataType: 'json',
//     beforeSend:function(){
//       $('.feedback1').text('Aligning unaligned centre(s) to respective province(s)');
//     },
//     success:function(data){
//       if(data.status == '200'){
//         // $('button[id=save]').attr('disabled',false).removeClass('bg_att');
...
function align_sen(){
  $.ajax({
    ...
    dataType: 'json',
    beforeSend:function(){
      $('.feedback1').text('Aligning sen candidate(s) to respective marking centre(s)....');
    },
    success:function(data){
      if(data.status == '200'){
        $('.feedback2').text(data.response_msg);
...
$('select[name=province]').change(function(){
  var province = $(this).val().split(':'),
    ...
    if(province_code == 'all'){
      ...
      $('select[name=marking_centre]').attr('disabled',true);
    }else{
      $('select[name=centre]').attr('disabled',false);
    }
});
function get_marking_centre_for_extract(){
  $('select[name=province]').change(function(){
    var province = $(this).val().split(':'),
      province_code = province[0],
      ...
      if(province_code == 'all'){
        ...
        $('select[name=marking_centre]').attr({'disabled':false,
                                                'required':true});
        ...
        $('select[name=paper]').attr({'disabled':false,
                                                'required':true});
    $.ajax({
                url: 'php/get_marking_centre_for_extract.php',
                method: 'POST',
                ...
                if(data.status != '400'){
                  ...
                  $('select[name=marking_centre]').append(
							...
							'<option value ="all">ALL MARKING CENTRES</option>'
						);
						$.each(data,function(){
							$('select[name=marking_centre]').append(
							'<option value ="'+this["marking_centre_code"]+'">'+this["marking_centre_code"]+' - '+this["marking_centre_name"]+'</option>'
						);
						});
...
function get_schools_in_marking_centre(){
  $('select[name=marking_centre]').change(function(){
    ...
    $.ajax({
          ...
          '<option value ="" selected disabled>Select Examination Centre</option>'+
          '<option value ="all" >ALL EXAMINATION CENTRES </option>'
        );
        $.each(data,function(){
          $('select[name=centre_code]').append(
  ...
  function upload_schools(){
    $('#ocrs_import_form').submit(function(e){
      var centres_type = $('input[type=hidden][name=centre_type]').val(),
        centres = centres_type == 'E' ? 'Grade 9 External' : 'Grade 9 Internal';
      e.preventDefault();
      $.ajax({
