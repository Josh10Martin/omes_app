...
$pdf = new Dompdf($options);
    ...
    if(isset($_POST['subject']) && isset($_POST['paper']) && isset($_POST['centre_type'])){
        ...
        $sql = $db_9->prepare('SELECT su.subject_code AS subject_code,su.subject_name AS subject_name, pa.paper_no AS paper_no,
                                 COUNT(m.exam_no) AS no_of_candidates,
                                --  TRIM(LEADING "0" FROM m.mark) AS mark, 
                                CAST(m.mark AS UNSIGNED) AS mark,
                                 (SELECT COUNT(*) FROM marks WHERE subject_code =:subject_code AND paper_no =:paper_no AND marking_centre =:marking_centre_code AND province =:province_code) AS total_no_of_candidates,                                  
                                 (SELECT COUNT(*) FROM marks WHERE subject_code =:subject_code AND paper_no =:paper_no AND marking_centre =:marking_centre_code AND province =:province_code AND status ="L") AS no_of_missing_mark, 
                                 (SELECT COUNT(*) FROM marks WHERE subject_code =:subject_code AND paper_no =:paper_no AND marking_centre =:marking_centre_code AND province =:province_code AND status ="X")AS no_of_absentees,
                                 (SELECT mark FROM marks WHERE subject_code =:subject_code AND paper_no =:paper_no AND marking_centre =:marking_centre_code AND province =:province_code AND status NOT IN ("X","L") ORDER BY CAST(mark AS UNSIGNED) DESC LIMIT 1) AS highest_mark,
                                 (SELECT mark FROM marks WHERE subject_code =:subject_code AND paper_no =:paper_no AND marking_centre =:marking_centre_code AND province =:province_code AND status NOT IN ("X","L") ORDER BY CAST(mark AS UNSIGNED) ASC LIMIT 1) AS lowest_mark 
                                ...
                                AND m.centre_code IN (SELECT centre_code FROM school WHERE centre_type =:centre_type AND province =:province_code)
                                AND m.marking_centre =:marking_centre_code
                                GROUP BY su.subject_code,su.subject_name,pa.paper_no,m.mark, CAST(m.mark AS UNSIGNED)
...
<html>
    ...
    <tr>
        <th>
        Subject: <?php echo $subject_code,' - ',subject_name($db_9,$subject_code); ?>
        </th>
    </tr>
    <tr>
        <th>
