...
$pdf = new Dompdf($options);
    ...
    if(isset($_POST['subject']) && isset($_POST['paper'])){
        ...
        $script_no =0;
        $array_nrc = array();
        $sql = $db_9->prepare('SELECT DISTINCT(ex.nrc) AS nrc, ex.tpin AS tpin, ex.title AS title, CONCAT(ex.first_name," ",ex.last_name) AS full_name, SUM(a.script_no) AS script_no,
                                ex.account_no AS account_no, mr.chief_examiner AS paper_rate, 
                                (SELECT (CASE WHEN SUM(DISTINCT(a.script_no)) = 0 THEN 0 WHEN SUM(DISTINCT(a.script_no)) < 100 THEN 100 ELSE SUM(DISTINCT(a.script_no)) END) AS no_of_scripts 
                                ...
                                ORDER BY SUM(DISTINCT(a.script_no)) DESC LIMIT 1) * (CASE WHEN ex.role = "5" THEN mr.chief_examiner ELSE mr.deputy_c_examiner END) / 

                                (SELECT COUNT(DISTINCT(ex.nrc))  AS no_of_examiners
                                FROM examiner ex INNER JOIN apportionment a ON (ex.subject_code = a.subject) 
                                WHERE ex.subject_code =:subject_code 
                                AND ex.paper_no =:paper_no 
                                AND ex.marking_centre =:marking_centre_code 
                                ...
                                ORDER BY SUM(DISTINCT(a.script_no)) DESC LIMIT 1) AS amount_claimed,
                               ...
                               GROUP BY ex.title,ex.first_name,ex.last_name,mr.chief_examiner,ex.role,mr.deputy_c_examiner,ex.nrc,ex.tpin,ex.account_no,b.name,br.name
                              ');
...
<html>
    <head>
        ...
        <title> CHIEF EXAMINERS / DEPUTY CHIEF EXAMINER</title>
    </head>
    <body>
        <style>
            table.center {
        ...
        <tr >
            ...
            <td>
               <p  style="text-align: center;">EXAMINATIONS COUNCIL OF ZAMBIA</p> 
                ...
                <p style="text-align: center;">CHIEF EXAMINER / DEPUTY CHIEF EXAMINER CLAIM</p> 
            </td>
        </tr>
    </table>
    ...
    <table class="bodered w100">
        ...
        <tbody >
            ...
            <tr>
                ...
                <td><?php echo number_format((float)$amount_claimed,2,'.',''); ?></td>
