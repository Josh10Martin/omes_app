...
$pdf = new Dompdf($options);
    ob_start();
    if((isset($_POST['deo']) || isset($_SESSION['username'])) && (isset($_POST['marking_centre']) || isset($_SESSION['marking_centre_code']))){
$username = $_POST['deo'] ?? $_SESSION['username'];
$marking_centre_code = $_POST['marking_centre'] ?? $_SESSION['marking_centre_code'];

    $sql = $db_9->prepare('SELECT marking_centre_name,full_name, bank,branch, account_no,
                            nrc,phone_number, no_of_scripts, gross_pay,15_wht,net_pay,
                            grossed_up_rate,session_name
                            FROM data_entry_claims
                            WHERE marking_centre_code =:marking_centre_code AND username =:username');
    $sql->execute(array(
