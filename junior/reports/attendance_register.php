...
$pdf = new Dompdf($options);
    ...
    if(isset($_POST['subject']) && isset($_POST['paper']) && isset($_POST['attendance']) && isset($_POST['belt_no'])){
        ...
        $sql = $db_9->prepare('SELECT ex.nrc AS nrc, CONCAT(ex.first_name," ",ex.last_name) AS full_name,
                                ex.no_of_days AS no_of_days,at.name AS attendance_value, ex.belt_no AS belt_no,
                                ...
                                INNER JOIN attendance at ON (ex.attendance = at.id)
        ...
        $sql->bindColumn('attendance_value',$attendance_value);
        ...
        $sql = $db_9->prepare('SELECT ex.nrc AS nrc, CONCAT(ex.first_name," ",ex.last_name) AS full_name,ex.no_of_days AS no_of_days,at.name AS attendance_value, su.subject_code AS subject_code, su.subject_name AS subject_name,p.name AS role,pa.paper_no,
                                ex.account_no AS account_no,b.name AS bank,br.name AS branch
                                FROM position p INNER JOIN examiner ex ON (p.id = ex.role)
                                INNER JOIN subjects su ON (ex.subject_code = su.subject_code)
                                INNER JOIN paper pa ON (su.subject_code = pa.subject_code)
