...
$pdf = new Dompdf($options);
    ...
    if(isset($_POST['subject']) && isset($_POST['paper'])){
        ...
        ex.account_no AS account_no, mr.examiner AS paper_rate, SUM(a.script_no) * mr.examiner AS amount_claimed,a.belt_no AS belt_no,
                               ...
                               GROUP BY ex.title,ex.first_name,ex.last_name,mr.examiner,ex.nrc,ex.account_no,b.name,br.name,a.belt_no
                              HAVING SUM(a.script_no) * mr.examiner = (SELECT SUM(a.script_no) * mr.examiner AS amount FROM apportionment a INNER JOIN marking_rates mr ON (a.subject = mr.subject_code) WHERE a.paper = mr.paper_no GROUP BY a.belt_no, mr.examiner ORDER BY amount DESC LIMIT 1)
                              ');
...
<html>
    <head>
        ...
        <title>SYSTEM ADMINS CLAIM</title>
    </head>
    <body>
        <style>
            table.center {
        ...
        <tr >
            ...
            <td>
               <p  style="text-align: center;">EXAMINATIONS COUNCIL OF ZAMBIA</p> 
                ...
                <p style="text-align: center;">SYSTEM ADMIN CLAIMS</p> 
            </td>
        </tr>
    </table>
...
$pdf->stream('SYSTEM ADMIN CLAIM', Array('Attachment'=>0));
