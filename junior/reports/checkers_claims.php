...
$pdf = new Dompdf($options);
    ...
    if(isset($_POST['subject']) && isset($_POST['paper']) && isset($_POST['app_belt_no'])){
        ...
        $grand_total = 0;
        // $no_of_examiners =0;
        $sql = $db_9->prepare('SELECT ex.title AS title, CONCAT(ex.first_name," ",ex.last_name) AS full_name,mr.checker AS paper_rate,SUM(a.script_no)
         AS script_no, mr.checker * (CASE WHEN SUM(a.script_no) = 0 THEN 0 WHEN SUM(a.script_no) < 100 THEN 100 ELSE SUM(a.script_no) END) AS amount_claimed,ex.account_no AS account_no,
                                b.name AS bank,br.name AS branch,(SELECT COUNT(*) FROM examiner WHERE marking_centre =:marking_centre_code AND subject_code =:subject_code AND paper_no =:paper_no AND belt_no =:belt_no AND role IN ("1","3")) AS no_of_examiners
                                ...
                                GROUP BY ex.title,ex.first_name,mr.checker,ex.last_name,ex.nrc,ex.account_no,b.name,br.name');
        $sql->execute(array(
...
p{
    ...
    <table class="head-data w80" style="margin-top: 5px;">
        <th>
            Subject Code: <?php echo $subject_code,' - ',subject_name($db_9,$subject_code); ?>
        </th>
        <th>
