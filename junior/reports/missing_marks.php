...
$pdf = new Dompdf($options);
    ob_start();
    if(isset($_POST['subject']) && isset($_POST['paper']) && isset($_POST['belt_number'])){
        ...
        $sql = $db_9->prepare('SELECT m.exam_no AS exam_no, m.first_name AS first_name,m.last_name AS last_name,m.centre_code AS centre_code,
                                m.status AS status, CASE WHEN m.sen =1 THEN "YES" WHEN m.sen =0 THEN "NO" ELSE "[UNKNOWN]" END AS sen, a.belt_no AS belt_no, su.subject_code AS subject_code,su.subject_name AS subject_name, pa.paper_no AS paper_no
                                FROM marks m INNER JOIN apportionment a ON (m.subject_code = a.subject)
