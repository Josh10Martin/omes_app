...
if(isset($_POST['exam_no']) && isset($_POST['subject_name']) && isset($_POST['paper_no'])){
        ...
        $sql = $db_9->prepare('DELETE FROM marks WHERE exam_no =:exam_no AND subject_code = (SELECT subject_code FROM subjects WHERE subject_name =:subject_name) AND paper_no =:paper_no AND improvised_mark = 1');
        $sql->execute(array(
                ':exam_no'=>$exam_no,
