...
if (isset($_POST['subject_code']) && isset($_POST['paper']) && isset($_POST['belt_no'])) {
    ...
    if ($_SESSION['user_type'] == 'ADMIN' || $_SESSION['user_type'] == 'DEO') {
        $sql = $db_9->prepare('SELECT DISTINCT e.nrc AS nrc, e.id AS id, e.first_name AS first_name, e.last_name AS last_name,
            e.phone_number AS phone, e.branch AS branch_id, su.subject_name AS subject_name,
            pa.paper_no AS paper_no, e.belt_no AS belt_no, po.name AS role
            FROM marking_centre_centres mcc
            INNER JOIN subjects su ON (mcc.subject_code = su.subject_code)
            ...
            WHERE e.subject_code = su.subject_code
              AND e.province = mcc.province
              AND e.marking_centre = mcc.marking_centre
              AND e.subject_code = mcc.subject_code
              AND e.paper_no = mcc.paper_no
              AND e.belt_no = :belt_no
              AND mcc.province = :province_code
              AND mcc.marking_centre = :marking_centre_code
              AND mcc.subject_code = :subject_code
              AND mcc.paper_no = :paper
            ORDER BY nrc ASC');
...
} elseif (isset($_POST['subject_code']) && isset($_POST['paper'])) {
    $subject_code = $_POST['subject_code'];
    $paper = $_POST['paper'];
    ...
    if ($_SESSION['user_type'] == 'ADMIN' || $_SESSION['user_type'] == 'DEO') {
        $sql = $db_9->prepare('SELECT DISTINCT e.nrc AS nrc, e.id AS id, e.first_name AS first_name, e.last_name AS last_name, e.phone_number AS phone, e.branch AS branch_id,
            su.subject_name AS subject_name, pa.paper_no AS paper_no, e.belt_no AS belt_no, po.name AS role
            FROM marking_centre_centres mcc
            INNER JOIN subjects su ON mcc.subject_code = su.subject_code
            INNER JOIN paper pa ON su.subject_code = pa.subject_code
            INNER JOIN examiner e ON pa.paper_no = e.paper_no
            INNER JOIN position po ON e.role = po.id
            WHERE e.subject_code = mcc.subject_code
              AND e.province = mcc.province
              AND e.marking_centre = mcc.marking_centre
              AND mcc.province = :province_code
              AND mcc.marking_centre = :marking_centre_code
