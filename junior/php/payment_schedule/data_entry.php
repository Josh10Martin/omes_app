...
if(isset($_SESSION['user_type']) && $_SESSION['user_type'] == 'DEO'){
        ...
        $sql = $db_9->prepare('SELECT marking_centre_name,nrc,tpin,full_name,position,no_of_scripts,bank,branch,sortcode,account_no,net_rate,grossed_up_rate,gross_pay,15_wht,net_pay,session_name
                                FROM data_entry_claims WHERE username =:username AND marking_centre_code =:marking_centre_code
                       ');
$sql->execute(array(
...
if($sql->rowCount() > 0){
        ...
        while($row = $sql->fetch(PDO::FETCH_ASSOC)){
                ...
                $data_array[$i]['subject_code'] = '';
                $data_array[$i]['subject_name'] =   '';
                $data_array[$i]['paper_no'] =  '';
                $data_array[$i]['belt_no'] = '';
