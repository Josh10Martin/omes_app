...
if($_SESSION['session_type']=='I'){
        if(isset($_POST['district']) && isset($_POST['subject_code'])){
                ...
                if(is_array($subject_codes) || is_array($districts)){
                        ...
                        AND ss.centre_code IN (SELECT centre_code FROM marks_prep WHERE subject_code IN (".$in_subjects.") AND apportion_id IN (SELECT apportion_id FROM apportionment_summary WHERE province = ? AND valid =1))
                        ORDER BY sc.centre_code ASC");
                $sql->execute([$province,$province]);
        ...
        if(isset($_POST['district'])){
                ...
                if(is_array($districts)){
                        ...
                        AND centre_code IN (SELECT centre_code FROM marking_centre_centres WHERE valid = 1)
                        AND centre_type = ?
