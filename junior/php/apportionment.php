...
if(isset($_POST['belt_no'])){
        ...
        $centre_in_belt = centre_in_belt($db_9,$subject_code,$paper_no,$school,$_SESSION['marking_centre_code'],$_SESSION['province_code']);
        ...
        }else if(school_exists($db_9,$school) == 'false'){
                $data_array['status'] = '400';
                $data_array['response_msg'] = 'Centre '.$school.' entered is not found';
        
        }else{
                try{
                $sql = $db_9->prepare('INSERT IGNORE INTO apportionment(group_id,subject,paper,belt_no,script_no,school,marking_centre,province,username,date_apportioned)
                                        VALUES(:id,:subject_code,:paper_no,:belt_no,:script_no,:school,:marking_centre_code,:province_code,:username,NOW())');
                $sql->execute(array(
